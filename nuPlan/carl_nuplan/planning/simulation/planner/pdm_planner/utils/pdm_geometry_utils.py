...
def convert_relative_to_absolute_se2_array(
    ...
    """
    Converts an StateSE2 array from relative to global coordinates.
    :param origin: origin pose of relative coords system
    :param state_se2_array: array of SE2 states with (x,y,θ) in last dim
    :return: SE2 coords array in relative coordinates
    """
    ...
    state_se2_abs[..., :2] = state_se2_array[..., :2] @ R.T + origin_array[..., :2]
    state_se2_abs[..., 2] = normalize_angle(state_se2_array[:, 2] + origin_array[..., 2])
...
def convert_relative_to_absolute_point_array(
    origin: StateSE2, point_array: npt.NDArray[np.float64]
) -> npt.NDArray[np.float64]:
    """
    Converts an points array from global to relative coordinates.
    ...
    points_abs = np.zeros_like(point_array, dtype=np.float64)
    points_abs[..., :2] = point_array[..., :2] @ R.T + origin_array[..., :2]
    return points_abs
