...
from carl_nuplan.planning.training.preprocessing.feature_builders.pdm_feature_builder import (
    get_ego_acceleration,
    get_ego_position,
    get_ego_velocity,
)
from carl_nuplan.planning.training.preprocessing.features.pdm_feature import (
    PDMFeature,
)
...
def create_pdm_feature(
    ...
    indices = sample_indices_with_time_horizon(num_past_poses, past_time_horizon, history.sample_interval)
    ...
    centerline_progress_values = (
        np.arange(model.centerline_samples, dtype=np.float64) * model.centerline_interval + current_progress
    ...
    if closed_loop_trajectory is not None:
        ...
        future_time_points: List[TimePoint] = [
            current_time + future_step_time * (i + 1) for i in range(model.trajectory_sampling.num_poses)
        ]
