...
class WithinBound:
    ...
    min_bound: float = float(-np.inf)
    max_bound: float = float(np.inf)

    def eval(self) -> npt.NDArray[np.bool_]:
        metric_values = np.array(self.metric)
        metric_within_bound = (metric_values > self.min_bound) & (metric_values < self.max_bound)
...
def derived_compute_lon_acceleration(
    ...
    return WithinBound(lon_acceleration, min_bound=min_lon_accel, max_bound=max_lon_accel)


def derived_compute_lat_acceleration(
    states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64]
) -> WithinBound:
    """
    ...
    return WithinBound(lat_acceleration, min_bound=-max_abs_lat_accel, max_bound=max_abs_lat_accel)


def derived_compute_jerk_metric(states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64]) -> WithinBound:
    """
    ...
    return WithinBound(jerk_metric, min_bound=-max_abs_mag_jerk, max_bound=max_abs_mag_jerk)


def derived_compute_lon_jerk_metric(
    states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64]
) -> WithinBound:
    """
    ...
    return WithinBound(lon_jerk_metric, min_bound=-max_abs_lon_jerk, max_bound=max_abs_lon_jerk)


def derived_compute_yaw_accel(states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64]) -> WithinBound:
    """
    ...
    """
    n_batch, n_time, n_states = states.shape
    yaw_accel_metric = _extract_ego_yaw_rate(states, time_steps_s, deriv_order=2, poly_order=2)
    return WithinBound(yaw_accel_metric, min_bound=-max_abs_yaw_accel, max_bound=max_abs_yaw_accel)


def derived_compute_yaw_rate(states: npt.NDArray[np.float64], time_steps_s: npt.NDArray[np.float64]) -> WithinBound:
    """
    ...
    return WithinBound(yaw_rate_metric, min_bound=-max_abs_yaw_rate, max_bound=max_abs_yaw_rate)


def ego_is_comfortable_debug(
    ...
    comfort_metric_functions = [
        derived_compute_lon_acceleration,
        derived_compute_lat_acceleration,
        derived_compute_jerk_metric,
        derived_compute_lon_jerk_metric,
        derived_compute_yaw_accel,
        derived_compute_yaw_rate,
    ]
    ...
    for idx, metric_function in enumerate(comfort_metric_functions):
        results[:, idx] = metric_function(states, time_point_s).eval()
