...
from carl_nuplan.planning.simulation.planner.pdm_planner.observation.pdm_observation import (
    PDMObservation,
)
from carl_nuplan.planning.simulation.planner.pdm_planner.observation.pdm_occupancy_map import (
    PDMOccupancyMap,
)
from carl_nuplan.planning.simulation.planner.pdm_planner.scoring.pdm_comfort_metrics import (
    ego_is_comfortable,
)
from carl_nuplan.planning.simulation.planner.pdm_planner.scoring.pdm_scorer_utils import (
    get_collision_type,
)
...
class PDMScorer:
    ...
    def _reset(
        ...
        self._ego_coords = state_array_to_coords_array(states, initial_ego_state.car_footprint.vehicle_parameters)
    ...
    def _calculate_ego_area(self) -> None:
        ...
        in_polygons = in_polygons.reshape(len(self._drivable_area_map), n_proposals, n_horizon, n_points).transpose(
            1, 2, 0, 3
        )  # shape: n_proposals, n_horizon, n_polygons, n_points

        drivable_area_on_route_idcs: List[int] = [
            idx for idx, token in enumerate(self._drivable_area_map.tokens) if token in self._route_lane_dict.keys()
        ...
        batch_oncoming_traffic_mask = center_in_polygon[..., drivable_area_on_route_idcs].sum(axis=-1) == 0
            ...
            for step_idx, future_time_idx in enumerate(future_time_idcs):
                ...
                for proposal_idx, geometry_idx in zip(intersecting[0], intersecting[1]):
                    ...
                    if is_agent_ahead(ego_rear_axle, track_state) or (
                        (
                            ego_in_multiple_lanes_or_nondrivable_area
                            or self._map_api.is_in_layer(ego_rear_axle, layer=SemanticMapLayer.INTERSECTION)
                        )
    ...
    def _calculate_driving_direction_compliance(self) -> None:
        ...
        cum_progress[:, 1:] = ((center_coordinates[:, 1:] - center_coordinates[:, :-1]) ** 2.0).sum(axis=-1) ** 0.5
        ...
        for proposal_idx in range(self._num_proposals):
            ...
            max_oncoming_traffic_progress = max(
                oncoming_progress.sum() for oncoming_progress in oncoming_progress_splits
            )
