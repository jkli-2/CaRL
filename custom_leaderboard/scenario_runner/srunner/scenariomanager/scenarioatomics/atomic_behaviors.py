...
class ActorFlow(AtomicBehavior):
    ...
    def update(self):
        ...
        # debug visualization
        # wp = self._sink_location + carla.Location(z=2.0)
        # color = carla.Color(0, 0, 0)  # Green
        # world = CarlaDataProvider.get_world()
        # world.debug.draw_point(wp, size=0.2, color=color, life_time=0.11)
        #
        # wp = self._source_location + carla.Location(z=2.0)
        # color = carla.Color(255, 255, 255)  # Green
        # world = CarlaDataProvider.get_world()
        # world.debug.draw_point(wp, size=0.2, color=color, life_time=0.11)
...
class OpenVehicleDoor(AtomicBehavior):
    ...
    def initialise(self):
        ...
        CarlaDataProvider.add_vehicle_with_open_door(self._actor, self._vehicle_door)
        super().initialise()
