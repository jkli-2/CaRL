...
class Accident(BasicScenario):
    ...
    def _spawn_side_prop(self, wp):
        ...
        while True:
            ...
            prop_wp = wp
            i += 1
            # NOTE There are cases where this can oscillate between two lanes. Break if too many tries were done.
            if i > 10:
                raise ValueError("Couldn't find an appropriate lane.")
    ...
    def _spawn_side_prop(self, wp):
        ...
        while True:
            ...
            if wp is None or wp.lane_type not in (carla.LaneType.Driving, carla.LaneType.Parking):
                break
            prop_wp = wp
            # Note added because this can endless loop in certain towns
            index += 1
            if index > 10:
                raise ValueError('Could not get an appropriate lane')
