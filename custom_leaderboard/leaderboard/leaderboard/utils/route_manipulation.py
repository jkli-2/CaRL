...
def interpolate_trajectory(waypoints_trajectory, hop_resolution=1.0, world_map=None):
    """
    ...
    grp = GlobalRoutePlanner(world_map, hop_resolution) # 33ms
    # Obtain route plan
    # Removed GPS to save a few ms every reset.
    route = []
    # 500 - 600 ms
    ...
    for i in range(len(waypoints_trajectory) - 1):
        ...
        interpolated_trace = grp.trace_route(waypoint, waypoint_next) # 1-20 ms
