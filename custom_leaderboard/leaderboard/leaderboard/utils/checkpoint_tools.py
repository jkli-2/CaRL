...
def save_dict(endpoint, data):
    if endpoint.startswith(('http:', 'https:', 'ftp:')):
        ...
        if proxies:
            _ = requests.patch(url=endpoint, headers={'content-type':'application/json'}, data=ujson.dumps(data, indent=4, sort_keys=True), proxies=proxies)
        else:
            _ = requests.patch(url=endpoint, headers={'content-type':'application/json'}, data=ujson.dumps(data, indent=4, sort_keys=True))
    else:
        with open(endpoint, 'w') as fd:
            ujson.dump(data, fd, indent=4)
