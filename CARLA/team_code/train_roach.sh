...
#SBATCH --output=/home/ste/Documents/CaRL/CARLA/results/logs/Roach_004_%a_%A.out
#SBATCH --error=/home/ste/Documents/CaRL/CARLA/results/logs/Roach_004_%a_%A.err
...
export PPO_CPP_DIR=/data/junkali/ppo.cpp
...
export LEADERBOARD_ROOT=${CUSTOM_LEADERBOARD_DIR}/leaderboard
export SCENARIO_RUNNER_ROOT=${CUSTOM_LEADERBOARD_DIR}/scenario_runner

export PYTHONPATH=$PYTHONPATH:${CARLA_ROOT}/PythonAPI
export PYTHONPATH=$PYTHONPATH:${CARLA_ROOT}/PythonAPI/carla
export PYTHONPATH="${SCENARIO_RUNNER_ROOT}":"${LEADERBOARD_ROOT}":${PYTHONPATH}

mkdir -p /scratch/junkali/cache/torch
...
echo "END TIME: $(date)"
printf 'Runtime: %dd:%dh:%dm:%ds\n' $((${runtime}/86400)) $((${runtime}%86400/3600)) $((${runtime}%3600/60)) $((${runtime}%60)) 2>&1 | tee ${WORK_DIR}/results/"${ex_name}"/train_time.txt
