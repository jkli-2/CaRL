...
#SBATCH --output=/weka/geiger/bjaeger25/ad_planning/2_carla/results/logs/AC_PPO_037_%a_%A.out
#SBATCH --error=/weka/geiger/bjaeger25/ad_planning/2_carla/results/logs/AC_PPO_037_%a_%A.err
#SBATCH --partition=h100-ferranti

start=`date +%s`
echo "START TIME: $(date)"
export SCENARIO_RUNNER_ROOT=/weka/geiger/bjaeger25/ad_planning/2_carla/custom_leaderboard/scenario_runner
export LEADERBOARD_ROOT=/weka/geiger/bjaeger25/ad_planning/2_carla/custom_leaderboard/leaderboard
...
echo "END TIME: $(date)"
printf 'Runtime: %dd:%dh:%dm:%ds\n' $((${runtime}/86400)) $((${runtime}%86400/3600)) $((${runtime}%3600/60)) $((${runtime}%60)) 2>&1 | tee /weka/geiger/bjaeger25/ad_planning/2_carla/results/"${ex_name}"/train_time.txt
