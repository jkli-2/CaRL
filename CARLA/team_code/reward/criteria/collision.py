'''
Contains a class that spawns a collision sensor in CARLA and reports collisions back.
'''
import carla
import weakref
import numpy as np
...
class Collision():
  '''
  Class that spawns a collision sensor in CARLA and reports collisions back.
  '''
  ...
  def __init__(self,
    ...
    self.min_area_of_collision = min_area_of_collision
    # If further than this distance, the area is forgotten
    self.max_area_of_collision = max_area_of_collision
    # Amount of time the last collision if is remembered
    self.max_id_time = max_id_time
  ...
  def tick(self, vehicle, timestamp):
    ...
    if self.last_id and timestamp - self.collision_time > self.max_id_time:
      self.last_id = None
  ...
  def _on_collision(weakself, event):
    ...
    for collision_location in self.registered_collisions:
      if ev_loc.distance(collision_location) <= self.min_area_of_collision:
        return
    # Ignore if its intensity is smaller than self.intensity_threshold
