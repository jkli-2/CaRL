...
class LeaderboardEvaluator(object):
    ...
    def _register_statistics(self, route_index, entry_status, crash_message=""):
        ...
        self.statistics_manager.compute_route_statistics(
            route_index, self.manager.scenario_duration_system, self.manager.scenario_duration_game, crash_message, self.route_date_string
        )
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        route_string = pathlib.Path(os.environ.get('ROUTES', '')).stem
        if args.collect_dataset:
            now = datetime.now()
            route_string += f'_route{config.index}'
            route_string = route_string + '_' + '_'.join(
                map(lambda x: '%02d' % x, (now.month, now.day, now.hour, now.minute, now.second)))
        self.route_date_string = route_string
        ...
        try:
            ...
            self.agent_instance.setup(args.agent_config, self.route_date_string)
    ...
    def run(self, args):
        ...
        # Turned off since we won't run anything after the code is done.
...
def main():
    ...
    parser.add_argument("--collect_dataset", type=int, default=0,
                        help="Whether the leaderboard is used to collect a dataset currently.")
