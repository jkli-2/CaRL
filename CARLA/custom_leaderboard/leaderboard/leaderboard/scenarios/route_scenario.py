...
class RouteScenario(BasicScenario):
    ...
    def __init__(self, world, config, debug_mode=0, criteria_enable=True):
        ...
        ego_vehicle = self._spawn_ego_vehicle() # 12 ms
        if ego_vehicle is None:
            raise ValueError("Shutting down, couldn't spawn the ego vehicle")


        self._get_parking_slots() # 40 ms
    ...
    def _get_route(self, config):
        ...
        self.route = config.keypoints # We precomputed the interpolation since its expensive
        return self.route
    ...
    def _filter_scenarios(self, scenario_configs):
        ...
        for scenario_number, scenario_config in enumerate(scenario_configs):
            # NOTE We already filter scenarios during generation
    ...
    def _spawn_ego_vehicle(self):
        ...
        # Local transform because route should be used as read only.
        local_transform = carla.Transform(elevate_transform.location, elevate_transform.rotation)
        local_transform.location.z += 0.5
        ...
        spectator.set_transform(carla.Transform(local_transform.location + carla.Location(z=50),
    ...
    def _create_behavior(self):
        ...
        behavior.add_child(scenario_triggerer)  # Tick the Scenario Trigger before the scenarios
    ...
    def _create_test_criteria(self):
        ...
        # NOTE We disable criteria for computing the DS etc. since the reward function does that job.
    ...
    def _create_weather_behavior(self):
        ...
        """
        if len(self.config.weather) == 1 or len(self.config.weather) == 0:
    ...
    def _initialize_environment(self, world):
        ...
        Set the weather
        """
        # Do not set any weather if none is specified. Useful for privileged training.
        if len(self.config.weather) == 0:
            return
