...
# NOTE I change the return of these 2 function from '' to 'None' because the return value is used as dictionary key
# '' is not a valid identifier so using it as a dictionary key in python can lead to a crash.
def get_lane_key(waypoint):
    ...
    return 'None' if waypoint is None else get_road_key(waypoint) + '*' + str(waypoint.lane_id)
...
def get_road_key(waypoint):
    ...
    return 'None' if waypoint is None else str(waypoint.road_id)
...
class BackgroundBehavior(AtomicBehavior):
    ...
    def __init__(self, ego_actor, route, debug=False, name="BackgroundBehavior"):
        ...
        self._ego_key = ""
        self._route_index = 0
        self._get_route_data(route) # 300 ms
    ...
    def _get_route_data(self, route):
        ...
        prev_trans = None
        # 100 ms for get_waypoint, Seems like it is not possible to pickle or store waypoints on disc.
        ...
        CarlaDataProvider.set_map_route(self._route)
    ...
    def _update_road_sources(self):
        ...
        for lane_key in self._road_dict:
            ...
            if distance > self._road_spawn_dist:
                # For debugging without vehicles
    ...
    def _extent_road_exit_space(self, space):
        ...
        for exit_lane_key in route_lane_keys:
            if exit_lane_key in junction.exit_dict:  # NOTE modified check if key exists.
    ...
    def _spawn_actors(self, spawn_wps, ego_dist=0):
        ...
        num_actors = len(spawn_transforms)
        # For debugging, we can turn off car spawning.
        if int(os.environ.get('NO_CARS', 0)) == 1:
            num_actors = 0
        actors = CarlaDataProvider.request_new_batch_actors(
            'vehicle.*', num_actors, spawn_transforms, True, False, 'background',
    ...
    def _update_road_actors(self):
        ...
        for lane_key in self._road_dict:
            for i, actor in enumerate(self._road_dict[lane_key].actors):
                ...
                # TODO Perf can cause up to 2 ms overhead with many vehicles.
                actor_wp = self._map.get_waypoint(location)
