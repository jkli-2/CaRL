...
def generate_target_waypoint_in_route(waypoint, route):
    ...
    # This will crash if the junction is not reached. TODO Test this
...
def get_closest_traffic_light(waypoint, traffic_lights=None, is_ego_wp=False):
    """
    ...
    # This corrects cases where autogenerated routes misidentify traffic lights, causing a red light on the ego's
    # path that never turns green.
    if is_ego_wp:
        waypoint_list = deque(maxlen=30)
        for _ in range(100):
            waypoint_list.append(waypoint)
            if waypoint.is_intersection:
                break
            waypoint = waypoint.next(1)
            if waypoint is None:
                break
            waypoint = waypoint[0]
        waypoint = waypoint_list[0]  # Take the waypoint 30 meters before the intersection
