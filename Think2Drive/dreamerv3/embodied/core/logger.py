...
class JSONLOutput(AsyncOutput):
    ...
    def __init__(self, logdir, filename="metrics.jsonl", pattern=r".*", strings=False, parallel=True):
    ...
    def __call__(self, summaries):
        ...
        for step, name, value in summaries:
            ...
            if isinstance(value, str):
                ...
                video_frames = _frames_to_rgb(value).transpose([0, 3, 1, 2])
                assert video_frames.shape[1] in [1, 3], video_frames.shape
                bystep[step][name] = wandb.Video(video_frames, fps=20, format="gif")
...
class ExpaOutput:
    ...
    def __init__(self, exp, run, project, user, config=None):
        try:
            ...
            self._expa = expa.Logger(exp, run, project, user, api_url="pubsub://expa-dev/ingest")
            if config:
...
def decompress_image(obs):
    ...
    decompressed_image = (
        255 * (1 & (image[..., None] >> np.arange(8, dtype=image.dtype)[None, None, None, :]))
    ).reshape((t, width, height, 40))

    return decompressed_image[..., :34].astype(np.uint8)


def _frames_to_rgb(frames):
    if frames.shape[-1] == 5:
        frames = decompress_image(frames)
    ...
    color_mappings = np.array(
        [
            ...
            [230, 50, 50],  # Yellow & Red traffic light at t=-5
            ...
            [230, 100, 100],  # Yellow & Red traffic light at t=-10
            ...
            [230, 150, 150],  # Yellow & Red traffic light at t=-15
...
def _encode_gif(frames, fps):
    ...
    Args:
        frames [n, width, height, channels]: Numpy array of frames.
        fps (float): Frames per second for the output GIF.

    Returns:
        bytes: The encoded GIF as bytes.
    """
    ...
    # 5: c_stop_sign
    # 6: ev_image
    video_frames = _frames_to_rgb(frames)
    ...
    # video frames is of shape (12, 384, 128, 3) = (frame, width, height, channels)
    with io.BytesIO() as output:
        # Save the frames as an animated GIF
        pil_frames[0].save(
            output, format="GIF", append_images=pil_frames[1:], save_all=True, duration=duration, loop=0, optimize=True
        )

        # Get the binary content of the GIF
