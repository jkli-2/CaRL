...
class Fifo:
    ...
    def __setitem__(self, key, stepids):
        ...
        Args:
            key: The item to be added to the queue.
            stepids: Unused in this implementation.
        """
    ...
    def __delitem__(self, key):
        ...
        Args:
            key: The item to be removed from the queue.
        """
...
class Uniform:
    """Uniform random selection implementation."""

    def __init__(self, seed=0):
        """
        Initialize the uniform selector with a given random seed.
    ...
    def __call__(self):
        ...
        Returns:
            A randomly selected key from the available keys.
        """
    ...
    def __delitem__(self, key):
        """
        Remove a key from the selector.
...
class Recency:
    """Recency-based selection implementation."""

    def __init__(self, uprobs, seed=0):
        """
        Initialize the recency selector with given probabilities and seed.

        Args:
            uprobs (list): List of probabilities for each recency level.
            seed (int): Seed for the random number generator. Default is 0.
        """
    ...
    def __call__(self):
        """
        Select and return an item based on recency.

        Returns:
            An item selected based on recency.

        Raises:
            KeyError: If unable to select an item after 10 retries.
        """
    ...
    def _sample(self, tree, rng, bfactor=16):
        """
        Sample an index from the tree.

        Args:
            tree (list): The probability tree.
            rng: Random number generator.
            bfactor (int): Branching factor of the tree. Default is 16.
    ...
    def _build(self, uprobs, bfactor=16):
        """
        Build the probability tree for sampling.
        ...
        Returns:
            list: The constructed probability tree.
        """
...
class Prioritized:
    ...
    def __init__(self, exponent=1.0, initial=1.0, zero_on_sample=False, maxfrac=0.0, branching=16, seed=0):
...
class LastK:
    # This implementation is specificly designed for replay.online = True. This ensures every sampled stepid is chosen
    # at least once after inserting it, independent on the specific sampling/selector strategy.
    def __init__(self, k, seed=0):
        self.k = k
        self.rng = np.random.default_rng(seed)
        self.eligible_keys = Uniform()
        self.untested_stepids_to_first_k_keys = defaultdict(list)
        self.all_keys = Uniform()

    def __call__(self):
        if self.eligible_keys.keys:
            return self.eligible_keys()
    ...
    def prioritize(self, stepids, priorities):
        ...
        for stepid, priority in zip(stepids, priorities):
            ...
            if is_terminal:
                for key in self.untested_stepids_to_first_k_keys[stepid]:
                    self.eligible_keys[key] = None

                del self.untested_stepids_to_first_k_keys[stepid]

    def __setitem__(self, key, stepids):
        if not isinstance(stepids[0], bytes):
            stepids = [x.tobytes() for x in stepids]

        self.all_keys[key] = stepids

        for stepid in stepids[: self.k]:
            self.untested_stepids_to_first_k_keys[stepid].append(key)

    def __delitem__(self, key):
        if key in self.eligible_keys.keys:
            del self.eligible_keys[key]

        del self.all_keys[key]


class Mixture:
    """Mixture of selectors implementation."""

    def __init__(self, selectors, fractions, seed=0):
        """
        Initialize the mixture selector with given selectors and fractions.

        Args:
            selectors (dict): Dictionary of selectors.
            fractions (dict): Dictionary of fractions for each selector.
            seed (int): Seed for the random number generator. Default is 0.
        """
    ...
    def __call__(self):
        """
        Select and return an item using a randomly chosen selector.

        Returns:
            The selected item from the chosen selector.
        """
        return self.rng.choice(self.selectors, p=self.fractions)()

    def __setitem__(self, key, stepids):
        """
        Add a new item to all selectors.

        Args:
            key: The key of the item to be added.
            stepids: Step IDs associated with the item.
        """
    ...
    def __delitem__(self, key):
        """
        Remove an item from all selectors.
    ...
    def prioritize(self, stepids, priorities):
        ...
        Args:
            stepids (list): List of step IDs to update.
            priorities (list): Corresponding priorities for each step ID.
        """
        for selector in self.selectors:
