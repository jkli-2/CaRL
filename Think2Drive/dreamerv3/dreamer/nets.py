...
class RSSM(nj.Module):
    ...
    def initial(self, bsize):
        carry = dict(
            deter=jnp.zeros([bsize, self.deter], f32), stoch=jnp.zeros([bsize, self.stoch, self.classes], f32)
        )
        if self.cell == "stack":
    ...
    def _gru(self, deter, stoch, action):
        ...
        if self.cell == "gru":
            ...
            for i in range(self.dynlayers):
                x = self.get(
                    f"dyn{i}", BlockLinear, self.deter, g, **kw, block_norm=self.block_norm, block_fans=self.block_fans
                )(x)
            x = self.get("dyncore", BlockLinear, 3 * self.deter, g, **self.kw, block_fans=self.block_fans)(x)
            ...
            for i in range(self.dynlayers):
                ...
                x = self.get(f"dyngru{i}core", Linear, 3 * deters[i].shape[-1], **self.kw)(x)
                reset, cand, update = jnp.split(x, 3, -1)
                reset = jax.nn.sigmoid(reset)
                cand = jnp.tanh(reset * cand)
                update = jax.nn.sigmoid(update - 1)
    ...
    def __call__(self, lat, bdims=2):
        ...
        if self.imgkeys:
            ...
            if self.space_hidden:
                ...
                x0 = einops.rearrange(x0, "... (g h w c) -> ... h w (g c)", h=self.minres, w=self.minres, g=g)
            ...
            for i, depth in reversed(list(enumerate(self.depths[:-1]))):
                x = self.get(f"conv{i}", Conv2D, depth, self.kernel, 2, **kw, transp=True)(x)
...
class Dist(nj.Module):
    ...
    def inner(self, inputs):
        ...
        if self.dist == "hyperbolic_mse":
            fwd = lambda x, eps=1e-3: (jnp.sign(x) * (jnp.sqrt(jnp.abs(x) + 1) - 1) + eps * x)
            bwd = lambda x, eps=1e-3: jnp.sign(x) * (
                jnp.square(jnp.sqrt(1 + 4 * eps * (eps + 1 + jnp.abs(x))) / 2 / eps - 1 / 2 / eps) - 1
            )
            return jaxutils.TransformedMseDist(out, len(self.shape), fwd, bwd)

        if self.dist == "symlog_and_twohot":
            bins = np.linspace(-20, 20, out.shape[-1])
            return jaxutils.TwoHotDist(out, bins, len(self.shape), jaxutils.symlog, jaxutils.symexp)
    ...
    def _layer(self, x):
        if self.transp:
            ...
            x = jax.lax.conv_transpose(
                x, kernel, (self.stride, self.stride), self.pad.upper(), dimension_numbers=("NHWC", "HWIO", "NHWC")
            )
        else:
            G = self.groups
...
class BlockLinear(nj.Module):
    ...
    def __init__(self, units, groups):
        ...
        self.groups = groups
        self._winit = Initializer(self.winit, self.outscale, self.fan, self.dtype, block_fans=self.block_fans)
        self._binit = Initializer("zeros", 1.0, self.fan, self.dtype)
        if self.block_norm:
    ...
    def __call__(self, x):
        ...
        if self.block_norm and self._norm != "none":
            x = jnp.concatenate([f(y) for f, y in zip(self._norm, jnp.split(x, self.groups, -1))], -1)
        else:
            x = self._norm(x)
        x = get_act(self.act)(x)
        return x
...
class Initializer:
    ...
    def __init__(self, dist="normal", scale=1.0, fan="in", dtype="default", block_fans=False):
        self.dist = dist
        self.scale = scale
        self.fan = fan
        self.dtype = dtype
