...
def cast_to_compute(values):
    return treemap(lambda x: x if x.dtype == COMPUTE_DTYPE else x.astype(COMPUTE_DTYPE), values)
...
class Optimizer(nj.Module):
    ...
    def __call__(self, modules, lossfn, *args, has_aux=False, **kwargs):
        ...
        if self.once:
            ...
            subcounts = {prefix: sum(v for k, v in counts.items() if k.startswith(prefix)) for prefix in set(prefs)}
    ...
    def _detailed_stats(self, optstate, params, updates, grads):
        ...
        for name, pattern in groups.items():
            ...
            if stddev is not None:
                ...
                pr = [jnp.abs(x) / jnp.maximum(1e-3, jnp.abs(y)) for x, y in zip(us, ps)]
                metrics.update(
                    {
                        f"{k}/{name}": v
                        for k, v in dict(
...
def expand_groups(groups, keys):
    ...
    groups = {group if group.endswith("/") else f"{group}/": value for group, value in groups.items()}
    assignment = {}
    ...
    for group in groups.keys():
        if not groupcount[group]:
            raise ValueError(f"Group {group} did not match any of the {len(keys)} keys.")
...
class SlowUpdater(nj.Module):
    ...
    def __call__(self):
        ...
        params = {k.replace(f"/{self.src.name}/", f"/{self.dst.name}/"): v for k, v in self.src.find().items()}
