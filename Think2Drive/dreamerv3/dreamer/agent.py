...
class Agent(nj.Module):
    ...
    def __init__(self, obs_space, act_space, config):
        ...
        enc_space = {
            ...
            and re.match(config.enc.spaces, k)
        }
        dec_space = {
            k: v
            for k, v in obs_space.items()
            if k not in ("is_first", "is_last", "is_terminal", "reward")
            and not k.startswith("log_")
            and re.match(config.dec.spaces, k)
        }
        ...
        # Actor
        kwargs = {}
        kwargs["shape"] = {k: (*s.shape, s.classes) if s.discrete else s.shape for k, s in self.act_space.items()}
        kwargs["dist"] = {
            k: config.actor_dist_disc if v.discrete else config.actor_dist_cont for k, v in self.act_space.items()
        }
        ...
        self.slowcritic = nets.MLP((), name="slowcritic", **self.config.critic, dtype="float32")
    ...
    def policy_keys(self):
        return "/(enc|dyn|actor|dec|con|critic|rew|retnorm|valnorm)/"
    ...
    def init_policy(self, batch_size):
        prevact = {k: jnp.zeros((batch_size, *v.shape), v.dtype) for k, v in self.act_space.items()}
    ...
    def policy(
        ...
        compressed_image = self.compress_image(obs) if compress_image else None
        obs = self.preprocess(obs)

        # Convert previous action to one-hot representation
        prevact = jaxutils.onehot_dict(prevact, self.act_space)

        def _compute_action(image):
            # Encode observation
            embed = self.enc({**obs, "image": image}, bdims=1)

            # Update latent state based on observation
            lat, out = self.dyn.observe(prevlat, prevact, embed, obs["is_first"], bdims=1)

            # Generate action distribution using actor network
            actor = self.actor(out, bdims=1)

            # Sample action from distribution
            act = sample(actor)

            max_value = jnp.nanmax(act["action"])

            return max_value, (obs, embed, lat, act, out)

        outs = {}
        if compute_saliency:
            image = obs["image"]
            grad_fn = jax.value_and_grad(_compute_action, has_aux=True)
            (_, (obs, embed, lat, act, out)), grads = grad_fn(obs["image"])
            saliency = jnp.abs(grads)
            saliency = (saliency - saliency.min()) / (saliency.max() - saliency.min())
            outs["saliency"] = saliency
        else:
            image = obs["image"]
            _, (obs, embed, lat, act, out) = _compute_action(image)

        if compute_rew:
            outs["estimated_reward"] = self.rew(
                {"stoch": out["stoch"], "deter": out["deter"]},
                bdims=1,
                training=False,
            ).mean()
        ...
        if compute_value:
            ...
            value = self.critic(out, bdims=1, training=False).mean()
            value = value * vscale + voffset
            outs["estimated_value"] = value

        if compute_cont:
            cont = self.con(out, bdims=1, training=False).mean()
            outs["estimated_cont"] = cont

        if compute_deconv:
            # Somehow setting bdims in self.dec() is not working, so we reshape its inputs
            dec = self.dec(
                {
                    "stoch": out["stoch"][None],
                    "deter": out["deter"][None],
                }
            )[
                "image"
            ].mode()[0]
            outs["estimated_dec"] = dec

        if self.config.replay_context:
            outs.update({k: out[k] for k in self.aux_spaces if k != "stepid"})
            outs["stoch"] = jnp.argmax(outs["stoch"], -1).astype(jnp.int32)

        # Check for finite values in various components
        d = dict(
            obs=obs,
            prevlat=prevlat,
            prevact=prevact,
        ...
        outs["finite"] = {
            ...
            for k, v in jax.tree_util.tree_leaves_with_path(d)
        }

        # Ensure all required auxiliary spaces are present
        assert all(k in outs for k in self.aux_spaces if k not in ("stepid", "finite", "is_online")), (
            list(outs.keys()),
            self.aux_spaces,
        )

        # Convert actions to appropriate format (discrete or continuous)
        act = {
            k: jnp.nanargmax(act[k], -1).astype(jnp.int32) if s.discrete else act[k] for k, s in self.act_space.items()
        }

        if compress_image:
            outs["compressed_image"] = compressed_image
    ...
    def train(self, data, carry):
        ...
        if self.config.replay_context:
            ...
            data = data.copy()
            context = {k: data.pop(k)[:, :K] for k in self.aux_spaces if k != "stepid"}
            context["stoch"] = f32(jax.nn.one_hot(context["stoch"], self.config.dyn.rssm.classes))
        ...
        if self.config.reset_context:
            keep = jax.random.uniform(nj.seed(), data["is_first"][:, :1].shape) > self.config.reset_context
            data["is_first"] = jnp.concatenate([data["is_first"][:, :1] & keep, data["is_first"][:, 1:]], 1)

        mets, (out, carry, metrics) = self.opt(
            self.modules, self.loss, data, carry, train_only_planner=False, has_aux=True
        )
        metrics.update(mets)
        self.updater()
        outs = {}
        ...
        if self.config.replay_context:
            ...
            outs["replay"].update({k: out["replay_outs"][k] for k in self.aux_spaces if k != "stepid"})
            outs["replay"]["stoch"] = jnp.argmax(outs["replay"]["stoch"], -1).astype(jnp.int32)

        # Compute priorities for prioritized replay if enabled
        # Make sure you either use the Prioritized selector or the last_k selector not both, otherwise priorities will be assigned wrong
        if self.config.replay.fracs.priority > 0:
            ...
            if self.config.replay.priosignal == "td":
                ...
                terms = [out[f"{k}_loss"] for k in ("rep", "dyn", *self.dec.veckeys, *self.dec.imgkeys)]
        ...
        # Compute priorities for last_k prioritized replay if enabled
        # Make sure you either use the Prioritized selector or the last_k selector not both, otherwise priorities will be assigned wrong
        if self.config.replay.fracs.last_k > 0:
            priority = data["is_terminal"]
            outs["replay"] = {"stepid": stepid, "priority": priority}
    ...
    def train_only_planner(self, data, carry):
        ...
        mets, (out, carry, metrics) = self.opt(
            self.modules, self.loss, data, carry, train_only_planner=True, has_aux=True
        )
        metrics.update(mets)
        self.updater()
        outs = {}
    ...
    def loss(self, data, carry, train_only_planner, update=True):
        metrics = {}
        prevlat, prevact = carry
        ...
        dists = dict(**self.dec(outs), reward=self.rew(rew_feat, training=True), cont=self.con(outs, training=True))
        ...
        if self.config.imag_start == "all":
            ...
            startlat = self.dyn.outs_to_carry(treemap(lambda x: x.reshape((B * T, 1, *x.shape[2:])), replay_outs))
            startout, startrew, startcon = treemap(lambda x: x.reshape((B * T, *x.shape[2:])), (replay_outs, rew, con))
        elif self.config.imag_start == "last":
            startlat = newlat
            startout, startrew, startcon = treemap(lambda x: x[:, -1], (replay_outs, rew, con))
        ...
        _, (outs, acts) = jaxutils.scan(
            imgstep, sg((startlat, startact)), jnp.arange(self.config.imag_length), self.config.imag_unroll
        )
        outs, acts = treemap(lambda x: x.swapaxes(0, 1), (outs, acts))
        outs, acts = treemap(
            lambda first, seq: jnp.concatenate([first, seq], 1),
            treemap(lambda x: x[:, None], (startout, startact)),
            (outs, acts),
        )
        ...
        actor_loss = sg(weight[:, :-1]) * -(logpi * sg(adv_normed) + self.config.actent * sum(ents.values()))
        ...
        losses["critic"] = (
            sg(weight)[:, :-1]
            * -(critic.log_prob(sg(ret_padded)) + self.config.slowreg * critic.log_prob(sg(slowcritic.mean())))[:, :-1]
        )

        if self.config.replay_critic_loss:
            replay_critic = self.critic(
                replay_outs if self.config.replay_critic_grad and not train_only_planner else sg(replay_outs)
            )
    ...
    def report(self, data, carry):
        ...
        # Train metrics
        _, (outs, carry_out, mets) = self.loss(data, carry, train_only_planner=False, update=False)
        metrics.update(mets)
        ...
        rec = dict(**self.dec(rec_outs), reward=self.rew(rec_outs), cont=self.con(rec_outs))
        img = dict(**self.dec(img_outs), reward=self.rew(img_outs), cont=self.con(img_outs))
        ...
        if self.config.report_gradnorms:
            for key in self.scales:
                try:
                    lossfn = lambda data, carry: self.loss(data, carry, update=False)[1][0][f"{key}_loss"].mean()
    ...
    def preprocess(self, obs):
        ...
        result["image"] = result["image"][..., :34]

        return result

    def compress_image(self, obs):
        image = obs["image"]

        assert len(image.shape) == 4, image.shape
        t, width, height, channels = image.shape

        mask = (image > 127).reshape((t, width, height, 5, 8))  # assuming channels=40
    ...
    def decompress_image(self, image):
        assert len(image.shape) == 5, image.shape

        t1, t2, width, height, channels = image.shape

        decompressed_image = (
            255 * (1 & (image[..., None] >> jnp.arange(8, dtype=image.dtype)[None, None, None, None, :]))
        ).reshape((t1, t2, width, height, 40))

        return decompressed_image[..., :40].astype(jnp.uint8)
