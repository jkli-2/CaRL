...
def main(argv=None):
    ...
    if config.task.split("_", 1)[0] == "carla":
    ...
    if args.script == "train":
        ...
        embodied.run.eval_only(bind(make_agent, config), bind(make_env, config), bind(make_logger, config), args)

    elif args.script == "eval_only_carla_parallel":
        embodied.run.eval_only_carla_parallel(bind(make_agent, config), bind(make_env, config), args, config)
    ...
    elif args.script == "parallel_carla":
        embodied.run.parallel_carla.combined(
            bind(make_agent, config),
            bind(make_replay, config, "replay", rate_limit=True),
            bind(make_env, config),
            bind(make_logger, config),
        ...
        embodied.run.parallel.env(bind(make_env, config), envid, args, False)

    elif args.script == "parallel_replay":
        embodied.run.parallel.replay(bind(make_replay, config, "replay", rate_limit=True), args)
        ...
        embodied.run.parallel_with_eval.parallel_env(bind(make_env, config), envid, args, True, is_eval)
...
def make_logger(config):
    ...
    logger = embodied.Logger(
        ...
        [
            ...
            embodied.logger.WandBOutput(project="master_thesis_rl", name=logdir.split("/")[-1]),
        ],
        multiplier,
    )
    return logger
...
def make_replay(config, directory=None, is_eval=False, rate_limit=False):
    ...
    if rate_limit and config.run.train_ratio > 0:
        kwargs["samples_per_insert"] = config.run.train_ratio / (length - config.replay_context)
        kwargs["tolerance"] = 5 * config.batch_size
        kwargs["min_size"] = min(max(config.batch_size, config.run.train_fill), size)
    ...
    if config.replay.fracs.uniform < 1 and not is_eval:
        ...
        # Changed, only use selectors that are relevant to improve efficiency
        fracs = config.replay.fracs
        selec = dict(
            uniform=selectors.Uniform(),
            priority=selectors.Prioritized(**config.replay.prio),
            recency=selectors.Recency(recency),
            last_k=selectors.LastK(config.replay.last_k),
        )
        selec = {key: value for key, value in selec.items() if fracs[key] != 0}
        fracs = {key: value for key, value in fracs.items() if value != 0}

        if len(selec) == 1:
            kwargs["selector"] = list(selec.values())[0]
        else:
            kwargs["selector"] = selectors.Mixture(selec, fracs)
...
def make_env(config, index, **overrides):
    ...
    kwargs["index"] = index  # Add the env index to the keyword dictionary
